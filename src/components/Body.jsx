...
const BodyComponent = () => {
  ...
  const fetchData = async () => {
    const data = await fetch(
      'https://corsproxy.io/?https://www.zomato.com/webroutes/getPage?page_url=/hyderabad/live-sports-screenings&location=&isMobile=0'
    );
        ...
        <button
          ...
          onClick={() => {
            filteredResData = resData.filter((res) => {
              return res?.name.toLowerCase().includes(searchKey.toLowerCase());
            });
        ...
        <button
          ...
          onClick={() => {
            const filteredData = resData.filter((res) => {
              return res?.rating?.aggregate_rating >= 4.3;
            });
